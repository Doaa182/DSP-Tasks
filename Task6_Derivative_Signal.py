def Run_Derivative_Signal():
    ...
    def get_first_derviative(x):   # y(n)=x(n)-x(n-1)
        output=[]
        output.append(1) # for  first element
        for n in range(1,len(x)-1):
            y=x[n]-x[n-1]
            output.append(y)
        return output
    
    def get_second_derviative(x):   # y(n)=x(n)-x(n-1)
        output=[]
        for n in range(1,len(x)-1):
            y=x[n+1]-(2*x[n])+x[n-1]
            output.append(y)
    ...
    FirstDrev=get_first_derviative(InputSignal)
    SecondDrev=get_second_derviative(InputSignal)
